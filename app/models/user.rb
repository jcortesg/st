...
class User < ActiveRecord::Base
  ...
  def referral_earnings(referrer_id = nil)
    ...
    #  earnings = earnings.where(referrer_id: referrer_id, user_id: self.id) unless referrer_id.blank?
    #  earnings = earnings.sum(:amount)
    #end

    earnings = Transaction.where("transaction_type = 'influencer_referrer_fee' or transaction_type = 'advertiser_referrer_fee'")
    earnings = earnings.where(referrer_id: referrer_id, user_id: self.id) unless referrer_id.blank?
    earnings = earnings.sum(:amount)
