...
class User < ActiveRecord::Base
  ...
  def referral_earnings
    ...
    if self.role == 'influencer'
      earnings = Transaction.where(transaction_type: 'influencer_referrer_fee', referrer_id: self.id).sum(:amount)
    end

    # Get the money generated by the advertiser
    if self.role == 'advertiser'
      earnings = Transaction.where(transaction_type: 'advertiser_referrer_fee', referrer_id: self.id).sum(:amount)
    end
  ...
  def total_referral_earnings
    Transaction.where("transaction_type = 'influencer_referrer_fee' or transaction_type = 'advertiser.referrer_fee'").
      where(referrer_id: self.referrals.collect {|r| r.id}).sum(:amount)
  end
